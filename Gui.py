    ...
    def tick(self) :
        ...
        if self.emp.clockedIn and (not self.emp.onLunch) and (not self.emp.onBreak) and (not self.emp.onTen) :
            ...
            if temp2 <= temp :
                ...
                if (self.emp.lastTime - pdate).days <= 6 :
                    if self.emp.totalHours1 > (self.emp.overweek.total_seconds() + self.emp.overtime1) :
                        self.label.config(text = round((self.emp.overtime + (datetime.datetime.now() - self.emp.lastTime).total_seconds()) / 3600.0 , 2))
                ...
                if emp.clockedIn and (not emp.onLunch):
                    ...
                    if (emp.hours + (datetime.datetime.now() - emp.lastTime).total_seconds() > datetime.timedelta(0,0,0,0,0,4).total_seconds()) and not emp.tookLunch:
                        if (emp.hours + (datetime.datetime.now() - emp.lastTime).total_seconds() > datetime.timedelta(0,0,0,0,0,6).total_seconds()):
                            self.insert('end' , 'May Be Working Illegaly')
                    ...
                    if (emp.hours + (datetime.datetime.now() - emp.lastTime).total_seconds() > datetime.timedelta(0,0,0,0,0,7).total_seconds()) and not emp.onSplit:
